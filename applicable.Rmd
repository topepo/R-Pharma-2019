...
rng <- extendrange(c(two_class_dat$A, two_class_dat$B))
...
nice_point <- data.frame(A = 2, B = 1.5)
...
p_tr_2 <- 
  ...
  xlab("Predictor A") + 
  ylab("Predictor B") + 
  geom_point(data = nice_point, col = "blue", cex = 5, pch = 15) + 
...
Implemented in the `applicable` package:
...
```{r tr-set-3, warning = FALSE, echo = FALSE}
print(
  p_tr + 
  geom_point(data = nice_point, col = "blue", cex = 5, pch = 15) + 
  geom_point(data = bad_point, col = "red", cex = 5)
...
```{r tr-pca, warning = FALSE, echo = FALSE}
ex_pca <- prcomp(two_class_dat[, 1:2], center = TRUE, scale. = TRUE)
ex_pca_dat <- as_tibble(ex_pca$x)
pca_rng <- extendrange(c(ex_pca_dat$PC1, ex_pca_dat$PC2))
pca_bad <- as_tibble(predict(ex_pca, bad_point))
pca_nice <- as_tibble(predict(ex_pca, nice_point))

p_pca_3 <-
  ggplot(ex_pca_dat, aes(x = PC1, y = PC2)) +
  geom_point(alpha = .2) +
  xlim(pca_rng) +
  ylim(pca_rng) +
  xlab("PC 1") +
  ylab("PC 2") +
  geom_point(data = pca_bad, col = "red", cex = 5) +
  geom_point(data = pca_nice, col = "blue", cex = 5, pch = 15) + 
  ggtitle("Corresponding PCA Scores")
...
The new PCA scores reflect 1D changes while the distance to center captures all PC values. 

```{r pca-score-plot-1, warning = FALSE, echo = FALSE,  out.width = '80%'}
ggplot(ex_pca_dat, aes(x = PC1, y = PC2)) +
  geom_point(alpha = .2) +
  xlim(pca_rng) +
  ylim(pca_rng) +
  ...
  geom_point(data = pca_bad, col = "red", cex = 5) +
  geom_segment(
    aes(x = pca_bad$PC1, y = pca_bad$PC2, xend = 0, yend = 0),
    col = "blue",
    arrow = arrow(length = unit(0.1, "inches"))
  ) +
  geom_segment(
    aes(x = pca_bad$PC1, y = pca_bad$PC2, xend = 0, yend = pca_bad$PC2),
    col = "blue",
    arrow = arrow(length = unit(0.1, "inches"))
  ) +
  geom_segment(
    aes(x = pca_bad$PC1, y = pca_bad$PC2, xend = pca_bad$PC1, yend = 0),
    col = "blue",
    arrow = arrow(length = unit(0.1, "inches"))
  ) 
```
...
Our reference distirbution is based on the within-training results. 

```{r pca-dist-ref, warning = FALSE, echo = FALSE,  out.width = '80%'}
ggplot(ex_pca_dat, aes(x = PC1, y = PC2)) +
  geom_point(alpha = .2) +
  xlim(pca_rng) +
  ylim(pca_rng) +
...
Let's look at the empirical distribution function of the first 2 components

```{r pca-plot, fig.width=6, fig.height=3,  out.width = '100%', fig.align='center', dev = 'svg', dev.args = list(bg = "transparent")}
autoplot(mutagen_domain, 1:2)
```

Other `dplyr` selectors (e.g. `starts_with()`) can be used.
...
For a training set of size _n_, there are _n_ similarity statistics for each new sample.
...
Thanks to Marly for all of her hard work. 

`applicable` uses Davis Vaughan's `hardhart` package, which made our lives much easier. 

The package is at [`https://github.com/tidymodels/applicable`](https://github.com/tidymodels/applicable).

These slides are at [`https://github.com/topepo/R-Pharma-2019`](https://github.com/topepo/R-Pharma-2019)

]
.pull-right[
<img src="images/goodbye.jpg" width = "60%">
